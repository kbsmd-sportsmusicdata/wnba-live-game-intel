...
PULL_CLEAN = load_module("pbpstats_2026_pull_clean", ROOT / "04_refresh" / "pbpstats_2026_pull_clean.py")
FEATURES = load_module("pbpstats_2026_features", ROOT / "04_refresh" / "pbpstats_2026_features.py")
...
class FeaturesHelpersTest(unittest.TestCase):
    ...
    def test_build_workflow_summary_has_no_game_level_note_and_team_games_table(self):
        raw_team = pd.DataFrame(
            [
                {"TeamAbbreviation": "ATL", "GamesPlayed": 10},
                {"TeamAbbreviation": "MIN", "GamesPlayed": 12},
            ]
        )
        clean_team = pd.DataFrame(
            [
                {"team_abbreviation": "ATL", "games_played": 10},
                {"team_abbreviation": "MIN", "games_played": 12},
            ]
        )
        feature_team = pd.DataFrame(
            [
                {"team_abbreviation": "ATL", "games_played": 10},
                {"team_abbreviation": "MIN", "games_played": 12},
            ]
        ...
        markdown = FEATURES.build_workflow_summary_markdown(
            raw_team_df=raw_team,
            clean_team_df=clean_team,
            feature_team_df=feature_team,
            run_id="20260610T000000Z",
        )

        self.assertIn("No game-level datasets are produced by this PBPStats totals pipeline.", markdown)
        self.assertIn("| ATL | 10 | 10 | 10 |", markdown)
        self.assertIn("| MIN | 12 | 12 | 12 |", markdown)
