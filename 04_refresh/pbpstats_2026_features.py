...
DEFAULT_DATA_ROOT = REPO_ROOT / "data" / "pbpstats_wnba_2026"
DATA_ROOT = Path(os.getenv("PBPSTATS_PIPELINE_DATA_ROOT", str(DEFAULT_DATA_ROOT)))
...
TEAM_RAW_LATEST = RAW_LATEST_DIR / "team_totals_raw_latest.csv"
...
def normalize_team_games_df(df: pd.DataFrame, dataset_label: str) -> pd.DataFrame:
    team_col = find_first_column(list(df.columns), ["team_abbreviation", "TeamAbbreviation", "name", "Name"])
    games_col = find_first_column(list(df.columns), ["games_played", "GamesPlayed", "gp", "GP"])

    if not team_col or not games_col:
        return pd.DataFrame(columns=["team_abbreviation", dataset_label])

    summary = df[[team_col, games_col]].copy()
    summary.columns = ["team_abbreviation", dataset_label]
    summary["team_abbreviation"] = summary["team_abbreviation"].astype(str)
    summary[dataset_label] = pd.to_numeric(summary[dataset_label], errors="coerce").astype("Int64")
    return summary


def build_team_games_summary(raw_team_df: pd.DataFrame, clean_team_df: pd.DataFrame, feature_team_df: pd.DataFrame) -> pd.DataFrame:
    merged = normalize_team_games_df(raw_team_df, "raw_team_totals")
    for df, label in [
        (clean_team_df, "clean_team_totals"),
        (feature_team_df, "feature_team_features"),
    ]:
        current = normalize_team_games_df(df, label)
        merged = merged.merge(current, on="team_abbreviation", how="outer")

    if merged.empty:
        return merged

    merged = merged.sort_values("team_abbreviation").reset_index(drop=True)
    return merged
...
def build_workflow_summary_markdown(
    *,
    ...
    clean_team_df: pd.DataFrame,
    feature_team_df: pd.DataFrame,
    run_id: str,
) -> str:
    lines = [
        "## PBPStats WNBA 2026 Summary",
        "",
        f"- Run ID: `{run_id}`",
        f"- Data root: `{DATA_ROOT}`",
        "",
        "### Game-Level Dataset Coverage",
        "",
        "No game-level datasets are produced by this PBPStats totals pipeline.",
        "",
        "### Team Games By Totals Dataset",
        "",
    ]

    games_df = build_team_games_summary(raw_team_df, clean_team_df, feature_team_df)
    if games_df.empty:
    ...
    lines.extend(
        [
            "| Team | Raw Team Totals | Clean Team Totals | Team Features |",
            "| --- | ---: | ---: | ---: |",
        ]
    )
    for _, row in games_df.iterrows():
        lines.append(
            f"| {row['team_abbreviation']} | {row['raw_team_totals']} | {row['clean_team_totals']} | {row['feature_team_features']} |"
        )
    return "\n".join(lines)
...
def main() -> None:
    ...
    raw_team = pd.read_csv(TEAM_RAW_LATEST) if TEAM_RAW_LATEST.exists() else pd.DataFrame()
    ...
    run_summary = {
        ...
        "workflow_summary": {
            "game_level_datasets": [],
            "team_games_by_dataset": build_team_games_summary(raw_team, team_clean, team_features).to_dict(orient="records"),
        },
    }
    ...
    write_github_step_summary(
        build_workflow_summary_markdown(
            raw_team_df=raw_team,
            clean_team_df=team_clean,
            feature_team_df=team_features,
            run_id=RUN_ID,
        )
    )
